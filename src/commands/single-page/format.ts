...
function getSinglePageListWidths(): number[] {
...
export function printSinglePageList(list: ListedSinglePageList, json = false): void {
  if (json) {
    printJson(list);
    return;
  }

  const widths = getSinglePageListWidths();
  ...
  for (const item of list.items) {
    table.push([
      item.page.metadata.name,
      truncateDisplayText(item.page.spec.title, widths[1]!),
      item.page.spec.publish ? "published" : "draft",
      formatTimestamp(item.page.metadata.creationTimestamp ?? undefined),
    ]);
  }
